#!/usr/bin/env python3
"""Stop hook for Worker delivery.

`kind: issue` is terminal. `kind: pr` loops until the PR is merged or
verify-blueprint fails and gives the agent CI-fix instructions.
"""
...
The stop hook cannot determine terminal delivery until `docs/delivery.yml` is deterministically parseable.

Current failure:
{diagnostic}

Task:
1. If you have not reached a delivery step yet, do not edit `docs/delivery.yml` prematurely. Resume from the interrupted point, strictly follow `AGENTS.md` and the active phase file, and proceed to `docs/deliver/pr.md` or `docs/deliver/issue.md` only when the workflow says to deliver.
2. If you are already in delivery or have opened the PR/issue, repair `docs/delivery.yml` so it is a single valid YAML mapping with exactly these five top-level keys and no duplicate keys: `kind`, `owner`, `repo`, `number`, and `url`.
3. The values must describe the terminal delivery for this run: `kind` must be either `pr` or `issue`; `owner` and `repo` must be non-empty; `number` must be a positive integer; `url` must be the PR or issue URL.
4. If no PR or issue exists yet and the workflow has reached delivery, follow the appropriate delivery file in `docs/deliver/`, then replace `docs/delivery.yml` with the exact delivery record.
5. Stop again only after `docs/delivery.yml` has one of these shapes:
...
def validate_delivery(workspace_dir: Path) -> Delivery:
    ...
    if kind not in TERMINAL_KINDS:
        errors.append("`kind` must be exactly `pr` or `issue`")
    ...
    if not isinstance(number, int) or isinstance(number, bool) or number <= 0:
        errors.append("`number` must be a positive integer PR/issue number")
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "leanmarathon-worker-stop-hook",
            }
        )
        if token:
...
6. Exit with message "PR_OPENED + PR_NUMBER_RECORDED". The orchestrator will detect the new HEAD and re-poll CI.
"""
