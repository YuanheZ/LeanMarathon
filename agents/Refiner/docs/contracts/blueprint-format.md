...
**Tool guidance:** When you need to inspect global definitional context for a target proof node, call `dag-tracker.global_definitional_context` with the target Lean name (for example, `{"lean_name":"T"}`). The tool returns all definitional nodes with their keyword and `visible`/`invisible` class; definitions remain excluded from `sorry_using`.
...
5. **Two-way `\cref` ↔ deps parity.** Let `C` = the set of Lean names of `lemma`/`theorem` nodes whose label appears in a `\cref{...}` citation inside `N`'s `proof` field. Let `S` (the **deps** set) be:
   ...
   - **Complete proof body**: `S` = the in-blueprint lemma/theorem dependencies reported by Lean elaborator metadata.
...
For every `lemma` node `L`, some later `lemma` or `theorem` must "use" `L`. A node `N` uses `L` if `L` appears in `N`'s in-blueprint Lean-elaborated proof dependencies. For placeholder bodies, this is the `sorry_using` list; for complete proof bodies, this is collected from elaborator metadata.
