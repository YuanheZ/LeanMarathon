...
- **Honesty About Source.** The proof text must faithfully represent the source proof's actual content--**no more, no less**. If the source contains a gap, unjustified claim, or flawed step, do not silently repair it, which contradicts the **No mathematical repair** boundary in `AGENTS.md`. Instead, keep and isolate it as a node with honestly recorded dependency.
